...
async function loadConfig() {
  ...
  cfg.endpoint = (r.endpoint || DEFAULT_ENDPOINT).replace(/\/+$/, "");
  cfg.token = r.token || "";
}
...
async function sendEnvelope(env) {
  if (!cfg.token) { console.warn("Teleporter: no token set (open the extension Options)"); return false; }
  try {
...
async function boot() { await loadConfig(); pollLoop(); }
boot();
